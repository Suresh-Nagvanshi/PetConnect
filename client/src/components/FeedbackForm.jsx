    ...
    <div className={`flex space-x-1 ${error ? 'border-red-500 border rounded-lg p-2' : ''}`}>
      {stars.map((star) => (
        <svg
          ...
          strokeWidth="1.5"
          className={`w-7 h-7 ${!readonly ? 'cursor-pointer hover:text-amber-400' : ''} text-gray-300 transition-colors`}
        >
...
const SuccessModal = ({ setShowModal, title = "Feedback Submitted!", message = "Thank you for helping us improve PetConnect. We value your input!" }) => (
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {displayTestimonials.map((testimonial, index) => (
        <div key={index} className="backdrop-blur-sm bg-white/90 rounded-xl md:rounded-2xl p-5 md:p-6 border border-white/30 shadow">
          <p className="text-gray-700 italic">"{testimonial.text}"</p>
          <div className="mt-3 text-sm text-gray-500">— {testimonial.author}</div>
        </div>
      ))}
    </div>
  );
...
function FeedbackForm({ 
  ...
  onSubmit,
  initialData = { name: '', email: '', rating: 0, feedback: '' },
      ...
      <div className={showModal ? 'filter blur-sm' : ''}>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-purple-700 text-white py-10 md:py-12 overflow-hidden -mx-6 md:-mx-10 -mt-6 md:-mt-10 rounded-none">
          <div className="absolute inset-0 z-0">
            <img src={backgroundImage} alt="Feedback background" className="w-full h-full object-cover opacity-50"/>
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
            <div className="backdrop-blur-md bg-white/10 rounded-2xl md:rounded-3xl p-6 md:p-8 border border-white/20 shadow-2xl">
              ...
              <p className="text-base md:text-lg text-blue-100">{subtitle}</p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className={`${showTestimonials ? 'lg:col-span-2' : 'lg:col-span-3'} backdrop-blur-sm bg-white/90 rounded-xl md:rounded-2xl p-6 md:p-8 border border-white/30 shadow`}>
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6" onSubmit={handleSubmit} noValidate>
              <div>
                ...
                <input 
                  ...
                  className={`w-full rounded-lg border p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.name ? 'border-red-500' : 'border-gray-300'}`} 
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={handleChange} 
                  className={`w-full rounded-lg border p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.email ? 'border-red-500' : 'border-gray-300'}`} 
              ...
              <div className="md:col-span-2">
                ...
                <textarea 
                  ...
                  onChange={handleChange} 
                  className={`w-full rounded-lg border p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.feedback ? 'border-red-500' : 'border-gray-300'}`} 
