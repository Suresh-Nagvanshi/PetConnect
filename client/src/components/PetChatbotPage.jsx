...
function PetChatbotPage() {
    ...
    // --- CHANGE #1: Create a ref for the chat window itself ---
    const chatWindowRef = useRef(null);
    // (The old messagesEndRef is no longer needed)
    // --------------------------------------------------------
    ...
    // --- CHANGE #2: Update the useEffect to scroll the chat window ---
    useEffect(() => {
        if (chatWindowRef.current) {
            // This tells the chat window element to set its own scroll position
            // to its maximum height, effectively scrolling it to the bottom.
            chatWindowRef.current.scrollTop = chatWindowRef.current.scrollHeight;
        }
    }, [messages]); // This still runs every time a new message is added
    // -------------------------------------------------------------
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { text: `🚨 ERROR: ${err.message}. Please try again.`, sender: 'bot' }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    const imagePreview = imageFile && (
        <div className="image-preview-card">
            ...
            <button className="remove-image-btn" type="button" title="Remove image" onClick={() => setImageFile(null)}>
                <TimesIcon />
            </button>
        </div>
    );
    ...
    return (
        <div className="chatbot-page-container">
            ...
            <div className="right-panel">
                {/* --- CHANGE #3: Attach the ref to the chat window div --- */}
                <div className="chat-window" ref={chatWindowRef}>
                    ...
                    {/* The old invisible div ref is now gone */}
                    {loading && (
