...
function SellerListAnimals() {
  ...
  const revokeAllPreviews = (urls) => {
    urls.forEach(u => {
      try { URL.revokeObjectURL(u); } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Form on the left */}
        <form onSubmit={addAnimal} className="lg:col-span-1 bg-white rounded-2xl p-6 shadow border border-gray-300">
          <h2 className="text-xl font-semibold mb-4">New Animal</h2>

          <label className="block mb-1">Name</label>
          <input name="name" value={form.name} onChange={handleChange} className="w-full border rounded mb-3 p-2" />

          <label className="block mb-1">Species</label>
          <input name="species" value={form.species} onChange={handleChange} className="w-full border rounded mb-3 p-2" />

          <label className="block mb-1">Age</label>
          <input name="age" value={form.age} onChange={handleChange} className="w-full border rounded mb-3 p-2" />

          <label className="block mb-1">Short Description</label>
          <textarea name="shortDescription" value={form.shortDescription} onChange={handleChange} rows="2" placeholder="A brief, catchy summary" className="w-full border rounded mb-3 p-2" />

          <label className="block mb-1">Long Description</label>
          <textarea name="longDescription" value={form.longDescription} onChange={handleChange} rows="4" placeholder="Full details about personality, habits, etc." className="w-full border rounded mb-3 p-2" />

          <label className="block mb-1">Images</label>
          <label htmlFor="animalImage" className="cursor-pointer inline-block mb-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
            Choose Files
          </label>
          <input
            id="animalImage"
          ...
          <button type="submit" className="w-full bg-blue-600 text-white rounded-lg py-3 font-semibold hover:bg-blue-700 transition">Add Animal</button>
        </form>
        ...
        <div className="lg:col-span-2 space-y-6 overflow-y-auto max-h-[80vh]">
          {animals.length === 0 ? (
            ...
            animals.map((a) => (
              <div key={a._id || a.id} className="bg-white rounded-2xl shadow border overflow-hidden flex w-full max-w-xl mx-auto">
                {a.imageUrls && a.imageUrls.length > 0 && (
                  <img 
                    src={`/uploads/${a.imageUrls[0]}`} 
                ...
                <div className="p-4 flex flex-col justify-center">
                  <h3 className="font-semibold text-xl">{a.petName} <span className="text-gray-400">({a.animalType})</span></h3>
                  {a.petAge && <p className="text-sm text-gray-500 mt-1">Age: {a.petAge}</p>}
                  {a.shortDescription && <p className="text-gray-700 mt-2 max-w-md">{a.shortDescription}</p>}
                </div>
              </div>
            ))
          )}
